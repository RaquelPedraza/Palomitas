    ...
    formCrearLista.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (data.ok) {
          ...
          cerrarModal("modalCrearListas");

          mostrarToast("Lista creada correctamente");
        }
      } catch (err) {
  ...
  if (btnEliminar) {
    btnEliminar.addEventListener("click", async () => {
      ...
      try {
        ...
        location.href = "mis_listas.phpphp#titulo-listas";
      } catch (err) {
        console.error(err);
      }
    });
...
let idPeliTemporal = null;
function prepararEliminacion(id) {
  idPeliTemporal = id;
  abrirModal("modalEliminarPelicula");
}
function ejecutarEliminacion() {
  eliminarPeliculaLista(idPeliTemporal);
  cerrarModal("modalEliminarPelicula");
}
